...
def LP_compression(X, h):
    ...
    print(f'optimum CR for LP is:{cr}')
...
def DCT_compression(X, N_values = [4, 8, 16]):
    ...
    print(f'optimum CR for DCT is:{Optimum_CR}')
...
#compare between scheme
def optimum_scheme(X):
    schemes = ['LP', 'DCT', 'LBT', 'DWT']
    CRs = []
    Zs = []
    for cs in schemes:
        if cs == 'DWT':
            CR1, Zq1 = DWT_compression(X)
            CRs.append(CR1)
            Zs.append(Zq1)
        if cs == 'LP':
            h_lp = np.array([1, 4, 6, 4, 1]) / 16.0
            CR2, Zq2 = LP_compression(X, h_lp)
            CRs.append(CR2)
            Zs.append(Zq2)
        if cs == 'DCT':
            CR3, Zq3 = DCT_compression(X)
            CRs.append(CR3)
            Zs.append(Zq3)
        if cs == 'LBT':
            CR4, Zq4 = LBT_compression(X)
            CRs.append(CR4)
            Zs.append(Zq4)

    Optimum_CR = CRs[np.argmax(CRs)]
    Optimum_sch = schemes[np.argmax(CRs)]
    Optimum_Z = Zs[np.argmax(CRs)]
    print(f'optimum CR for X is : {Optimum_CR}')
    print(f'optimum compression scheme for X is : {Optimum_sch}')
    return Optimum_CR, Optimum_sch, Optimum_Z

def main():
    X, cmaps_dict = load_mat_img(img='lighthouse.mat', img_info='X', cmap_info={'map', 'map2'})
    X = X - 128.0
    Optimum_CR, Optimum_sch, Optimum_Z = optimum_scheme(X)

    fig, ax = plt.subplots()
    plot_image(Optimum_Z, ax=ax)
    ax.set(title=f"{Optimum_sch}_lighthouse.mat")
    print(f"{Optimum_sch} compressed image saved")

    plt.savefig(f"{Optimum_sch}_lighthouse.png")
